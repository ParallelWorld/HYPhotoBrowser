...
//        }];
        ...
        [_imageView sd_setImageWithURL:_photo.url placeholderImage:_photo.srcImageView.image options:SDWebImageRetryFailed | SDWebImageLowPriority progress:^(NSInteger receviedSize, NSInteger expectedSize){
        
        
            if (receviedSize > kMinProgress) {
                if (!self.hidden)
                {
                     loading.progress = (float)receviedSize/expectedSize;
                }
